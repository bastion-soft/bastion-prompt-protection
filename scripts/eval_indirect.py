"""Indirect / agentic prompt-injection leaderboard — scored pure-model.

Scores the SAME baselines as ``scripts/run_leaderboard.py`` on the held-out
indirect / structured sets in ``eval/indirect_data.py`` (Z-Edgar, BIPIA,
InjecAgent, AgentDojo, HackAPrompt, TensorTrust), the same way as the direct
leaderboard (pure model via ``TransformersRunner``). Written to a SEPARATE
``results/indirect.json`` — these target structured/indirect injection, a distinct
capability, so they are NOT folded into the main 4-benchmark average.
...
from eval.indirect_data import INDIRECT_LOADERS
from eval.runners import TransformersRunner
from scripts.run_leaderboard import BASELINES, COMMERCIAL_MODELS
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        "(e.g. eval/results/scores_indirect) so indirect sets aren't pooled with direct attacks.",
    )
    return p.parse_args()
