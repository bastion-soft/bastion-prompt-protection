...
``scripts.analyze_operating_points`` (no model inference):
  ...
  * ``fpr_vs_threshold.svg`` — FPR on real benign vs decision threshold, one
    ...
    threshold-sensitive (steep) — the hlyn-vs-Bastion contrast.
...
def main() -> int:
    ...
    # Stable ordering; highlight Bastion if present.
    names = sorted(curves)

    def _style(name: str) -> dict:
        if name.lower().startswith("bastion"):
            return {"linewidth": 2.6, "zorder": 5}
        return {"linewidth": 1.3, "alpha": 0.85}
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--det-points", default="eval/results/det_points.json")
